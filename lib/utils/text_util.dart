...
class TextRobotoAutoNormal extends StatelessWidget {
  const TextRobotoAutoNormal(this.text, {super.key, this.maxLines, this.color, this.fontSize, this.textAlign, this.decoration});
  ...
  Widget build(BuildContext context) {
    return AutoSizeText(text,
        ...
        overflow: TextOverflow.ellipsis,
        textAlign: textAlign ?? TextAlign.start,
        style: Get.theme.textTheme.displaySmall!.copyWith(color: color, fontSize: fontSize, decoration: decoration));
  }
}
...
textWithCopyView(String text, {MainAxisAlignment? mainAxisAlign}) {
  return Row(
